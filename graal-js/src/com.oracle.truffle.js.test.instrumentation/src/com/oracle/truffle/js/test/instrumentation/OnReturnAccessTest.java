...
public abstract class OnReturnAccessTest extends FineGrainedAccessTest {

    private ArrayList<CharSequence> sources = new ArrayList<>();

    protected void evalExpressions(String src) {
        instrumenter.attachExecutionEventListener(SourceSectionFilter.newBuilder().tagIs(ExpressionTag.class).includeInternal(false).build(), new ExecutionEventListener() {
            @Override
