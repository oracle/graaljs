...
import com.oracle.truffle.js.builtins.wasm.WebAssemblyMemoryPrototypeBuiltinsFactory.WebAssemblyMemoryToFixedLengthBufferNodeGen;
...
public class WebAssemblyMemoryPrototypeBuiltins extends JSBuiltinsContainer.SwitchEnum<WebAssemblyMemoryPrototypeBuiltins.WebAssemblyMemoryPrototype> {
    ...
    protected Object createNode(JSContext context, JSBuiltin builtin, boolean construct, boolean newTarget, WebAssemblyMemoryPrototype builtinEnum) {
        switch (builtinEnum) {
            ...
            case toFixedLengthBuffer:
                return WebAssemblyMemoryToFixedLengthBufferNodeGen.create(context, builtin, args().withThis().createArgumentNodes(context));
            case toResizableBuffer:
                return WebAssemblyMemoryToResizableBufferNodeGen.create(context, builtin, args().withThis().createArgumentNodes(context));
            case buffer:
    ...
    public abstract static class WebAssemblyMemoryToFixedLengthBufferNode extends JSBuiltinNode {

        public WebAssemblyMemoryToFixedLengthBufferNode(JSContext context, JSBuiltin builtin) {
            super(context, builtin);
        }

        @Specialization
        protected JSArrayBufferObject toFixedLengthBuffer(JSWebAssemblyMemoryObject memory) {
            return memory.toFixedLengthBuffer(getContext(), getRealm());
        }

        @TruffleBoundary
        @Fallback
        protected Object doIncompatibleReceiver(@SuppressWarnings("unused") Object thisObj) {
            throw Errors.createTypeError("WebAssembly.Memory.toFixedLengthBuffer(): Receiver is not a WebAssembly.Memory", this);
        }
    }

    public abstract static class WebAssemblyMemoryToResizableBufferNode extends JSBuiltinNode {

        public WebAssemblyMemoryToResizableBufferNode(JSContext context, JSBuiltin builtin) {
            super(context, builtin);
        }

        @Specialization
        protected JSArrayBufferObject toResizableBuffer(JSWebAssemblyMemoryObject memory) {
            return memory.toResizableBuffer(getContext(), getRealm());
        }

        @TruffleBoundary
        @Fallback
        protected Object doIncompatibleReceiver(@SuppressWarnings("unused") Object thisObj) {
            throw Errors.createTypeError("WebAssembly.Memory.toResizableBuffer(): Receiver is not a WebAssembly.Memory", this);
        }
    }
