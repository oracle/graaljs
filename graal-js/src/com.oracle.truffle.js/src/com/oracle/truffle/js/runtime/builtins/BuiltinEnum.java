...
public interface BuiltinEnum<E extends Enum<? extends BuiltinEnum<E>>> {
    ...
    default Object getKey() {
        TruffleString key = stripName(Strings.fromJavaString(asEnum().name()));
        // Precompute hash code so that it is baked into the native image.
        key.hashCodeUncached(TruffleString.Encoding.UTF_16);
        return key;
    }
