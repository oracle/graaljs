...
abstract class AbstractDisposeResourcesNode extends JavaScriptBaseNode {
    ...
    protected final Object callDisposeMethod(DisposableResource resource) {
        if (resource.getDisposeMethod() == Undefined.instance) {
            return Undefined.instance;
        }
        Object[] args;
        if (resource.hasDisposeArgument()) {
            args = JSArguments.createOneArg(resource.getResourceValue(), resource.getDisposeMethod(), resource.getDisposeArgument());
        } else {
            args = JSArguments.createZeroArg(resource.getResourceValue(), resource.getDisposeMethod());
        }
        return callNode.executeCall(args);
    }

    protected final Object toErrorObject(Throwable throwable) {
        if (getErrorObjectNode == null) {
            CompilerDirectives.transferToInterpreterAndInvalidate();
            getErrorObjectNode = insert(TryCatchNode.GetErrorObjectNode.create());
        }
        return getErrorObjectNode.execute(throwable, true);
    }

    protected final Object captureDisposeError(Throwable throwable) {
        if (throwable instanceof AbstractTruffleException ex) {
            return toErrorObject(ex);
