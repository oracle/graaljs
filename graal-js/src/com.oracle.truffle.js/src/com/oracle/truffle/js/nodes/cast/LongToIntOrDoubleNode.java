...
/**
 * @see JSRuntime#longToIntOrDouble(long)
 */
@GenerateInline
@GenerateUncached
public abstract class LongToIntOrDoubleNode extends JavaScriptBaseNode {

    public abstract Number execute(Node node, long longValue);
    ...
    public final Number fromIndex(Node node, long index) {
        ...
        return execute(node, index);
    }

    @Specialization
    protected static Number toIntOrDouble(Node node, long value,
                    @Cached InlinedBranchProfile toDoubleBranch) {
        if (CompilerDirectives.injectBranchProbability(CompilerDirectives.LIKELY_PROBABILITY, JSRuntime.longIsRepresentableAsInt(value))) {
            return (int) value;
        } else {
