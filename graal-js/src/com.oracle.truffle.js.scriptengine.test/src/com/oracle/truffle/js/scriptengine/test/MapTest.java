...
public class MapTest {
    ...
    @Test
    /**
     * Regression test for github.com/graalvm/graaljs/issues/214.
     *
     * A JavaScript array, when passed to Java via ScriptEngine, ends up as (empty) Map, not as
     * List.
     */
    public void testJavaScriptArrayViaScriptEngine() throws ScriptException {
        ...
        // it is counter-intuitive that this is a Map, not a List
        Assert.assertFalse(result instanceof List);
        Assert.assertTrue(result instanceof Map);
        // especially, when this Map is completely empty
        Assert.assertEquals(0, ((Map<?, ?>) result).entrySet().size());

        // targetTypeMapping can be used to solve the problem

        HostAccess hostAccess = HostAccess.newBuilder(HostAccess.ALL).targetTypeMapping(List.class, Object.class, null, v -> v).build();
        engine = GraalJSScriptEngine.create(null, Context.newBuilder("js").allowHostAccess(hostAccess));
        result = engine.eval("['a', 'b', 'c']");
