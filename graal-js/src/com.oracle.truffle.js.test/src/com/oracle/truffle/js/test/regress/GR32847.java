...
public class GR32847 extends JSTest {
    ...
    @Test
    public void asyncThrowHostException() {
        try (Context context = Context.newBuilder().allowAllAccess(true).build()) {
            ...
            context.eval(Source.newBuilder("js", """
                            async function main() {
                                (function foo() {
                                    JavaKlass.throwException();
                                })();
                            }
                            """, "Unnamed").buildLiteral());

            context.getBindings("js").putMember("JavaKlass", new JavaException());

            Value asyncFn = context.getBindings("js").getMember("main");
            // (3)
            ...
            Consumer<Object> javaThen = (v) -> {
                ...
                try {
                    // (6) + (7)
                    throw context.asValue(v).throwException();
                } catch (Exception e) {
                    // (8)
