...
public class GR36607 {

    @Test
    public void test() {
        try (Context context = JSTest.newContextBuilder().build()) {
            try {
                context.eval(JavaScriptLanguage.ID, "(function myFunction() { (x=>x).caller })()");
                ...
                for (PolyglotException.StackFrame frame : ex.getPolyglotStackTrace()) {
                    ...
                    if (first) {
                        Assert.assertEquals("myFunction", rootName);
                        first = false;
                    } else {
                        Assert.assertNotNull(rootName);
                    }
                }
            }
        }
