...
public class AsyncIteratorFunctionBuiltinsTest {
    @Test
    ...
    public void testFrom() {
        ...
        try (Context context = builder.build()) {
            ...
            result = context.eval(JavaScriptLanguage.ID, "AsyncIterator.from({[Symbol.iterator]: () => ({next: () => ({done: true})})})");
            Assert.assertTrue(result.hasMembers());
            Assert.assertTrue(result.hasMember("next"));
            Assert.assertTrue(result.getMember("next").canExecute());
            Assert.assertTrue(result.hasMember("map"));
            ...
            result = context.eval(JavaScriptLanguage.ID, "AsyncIterator.from({next: () => ({done: true})})");
            Assert.assertTrue(result.hasMembers());
            Assert.assertTrue(result.hasMember("next"));
            Assert.assertTrue(result.getMember("next").canExecute());
            Assert.assertTrue(result.hasMember("map"));
