...
public class ConstructorBuiltinsInteropTest {
    ...
    private static void checkArrayWithForeignArgument(Object foreignArgument, int expectedLength) {
        try (Context context = JSTest.newContextBuilder().build()) {
            ...
            assertEquals(expectedLength, result.asInt());
        }
    }

    @Test
    public void testArrayWithForeignLength() {
        checkArrayWithForeignArgument(ForeignBoxedObject.createNew(42), 42);
    }

    @Test
    public void testArrayWithForeignArgument() {
        checkArrayWithForeignArgument(ForeignBoxedObject.createNew("42"), 1);
    }
    ...
    private static void checkArrayWithForeignLengthOutOfBounds(Object foreignNumber) {
        try (Context context = JSTest.newContextBuilder().build()) {
            ...
            String jscode = "var rangeErrorThrown = false;\n" +
                            "try {\n" +
                            "    new Array(foreignNumber);\n" +
                            "} catch (e) {\n" +
                            "    rangeErrorThrown = e instanceof RangeError;\n" +
                            "}\n" +
                            "rangeErrorThrown";
            Value result = context.eval(JavaScriptLanguage.ID, jscode);
    ...
    @Test
    public void testArrayWithForeignLengthOutOfBounds() {
