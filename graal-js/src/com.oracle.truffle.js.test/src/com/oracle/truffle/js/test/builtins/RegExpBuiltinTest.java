...
public class RegExpBuiltinTest {
    ...
    public void testUndefinedFlags() {
        try (Context context = JSTest.newContextBuilder().build()) {
            ...
            // undefined flags should be converted to an empty string
            // (even when Object specialization for flags is activated)
            flags = context.eval(JavaScriptLanguage.ID, "new RegExp('.').flags");
            Assert.assertTrue(flags.isString());
            Assert.assertEquals("", flags.asString());
        }
    }
