...
public class JSONBuiltinsTest {
    ...
    @Test
    public void testParseImmutable() {
        assertTrue(execute(
                "let text = '{ \"status\": 200, \"data\": [1, 2, 3], \"ignore\": 42}';",
                "let filter = (name, immutable) => name == 'ignore' ? undefined : immutable;",
                "JSON.parseImmutable(text, filter) === #{ status: 200, data: #[1, 2, 3] };"
        ).asBoolean());
    }
    ...
    public void testStringify() {
        assertTrue(execute(
                "let data = #{ status: 200, data: #[1, 2, 3] };",
                "JSON.stringify(data) === '{\"data\":[1,2,3],\"status\":200}';"
        ).asBoolean());
    }
}
