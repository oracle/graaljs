...
EXPORT_TO_JS(GetStartPosition) {
    Local<Message> message = createMessage(args);
    args.GetReturnValue().Set(message->GetStartPosition());
}

EXPORT_TO_JS(GetEndPosition) {
    Local<Message> message = createMessage(args);
    args.GetReturnValue().Set(message->GetEndPosition());
}
